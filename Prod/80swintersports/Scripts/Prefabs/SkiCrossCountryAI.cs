...
public partial class SkiCrossCountryAI : Node
{
    #region Behaviors
    ...
    #endregion
    #region Methos
    public List<CrossCountryModel> InstantiateArea3D(string parentName)
    {
        List<CrossCountryModel> crossCountryModelList = new List<CrossCountryModel>();
        var child = this.GetChildren();        

        var hillBaseList = child.Where(p => p.Name.ToString().Contains(parentName)).ToList();
        int count = 0;
