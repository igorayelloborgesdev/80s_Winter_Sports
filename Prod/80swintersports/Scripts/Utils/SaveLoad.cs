...
public class SaveLoad
{
    ...
    public static T LoadData<T>(string path)
    {
        try
        {
            var file1 = Godot.FileAccess.Open("res://" + path, Godot.FileAccess.ModeFlags.Read);
            if (file1 is not null)
            {                
                var jsonString = file1.GetAsText();
                return JsonConvert.DeserializeObject<T>(jsonString);
    ...
    public static T LoadData<T>(string path, string path1)
    {
        try
        {
            ...
            file1 = Godot.FileAccess.Open("res://" + path1, Godot.FileAccess.ModeFlags.Read);
            if (file1 is not null)
            {             
                var jsonString = file1.GetAsText();
                return JsonConvert.DeserializeObject<T>(jsonString);
            ...
            var file2 = File.ReadAllText(loadPath);
            return JsonConvert.DeserializeObject<T>(file2);
        }
        catch (Exception ex)
        {
            return default(T);
